...
setTime equ 1034h 	;Stores 0 or 1. 1 if time is set. 0 if not
...
mul bl
mov bx, ax
mov si, offset [A10MINUTE]      ; copy offset into interrupt vector:
mov es:[bx], si
add bx, 2
...
mul bl
mov bx, ax 
mov si, offset [A1MINUTE]   ; copy offset into interrupt vector:
mov es:[bx], si
add bx, 2
mov ax, 0000           ; copy segment into interrupt vector:
mov es:[bx], ax
...
mul bl
mov bx, ax
mov si, offset [A10SECOND]    ; copy offset into interrupt vector:
mov es:[bx], si
add bx, 2
mov ax, 0000         ; copy segment into interrupt vector:
mov es:[bx], ax
...
mul bl
mov bx, ax
mov si, offset [POWER]    ; copy offset into interrupt vector:
mov es:[bx], si
add bx, 2
mov ax, 0000        ; copy segment into interrupt vector:
mov es:[bx], ax
...
mov word ptr [sec],0 ;Initial Number of Seconds Set
mov word ptr [min],0 ;Initial Number of Minutes Set
mov byte ptr [power],-1 ;Number of Times Power Button is Pressed
mov byte ptr [start],0 ;NUmber of Times Start Button is Pressed
mov byte ptr [stop],00 ;Number of Times of Stop Button is Pressed
mov byte ptr [setTime],0 ;Timer has not been Set Initially 
mov byte ptr [inProgress],0 ;Cooking is not in Progress.

mov ax,00110110b 	 ;Control Word for Timer 2 Counter 0 (Mode 3 : Square Wave Generator)
mov dx,3006h         ;16 bit binary count value loaded as LSB first followed by MSB
out dx,al            

mov ax,20h			 ;Loading the count value for Timer 2 Counter 0 
mov dx,3000h         ;Least Significant Byte = 20h
out dx,al            ;Most Significant Byte = 4Eh
mov ax,4eh           ;4E20h = 20000d
mov dx,3000h        
out dx,al

mov ax,01010110b    		 ;Control Word for Timer 2 Counter 1 (Mode 3 : Square Wave Generator)
mov dx,3006h                 ;8 bit binary count value loaded into the MSB only
out dx,al

mov ax,100			 ;Loading Count value for Timer 2 Counter 1
mov dx,3002h         ;Most Significant Byte = 100d = 64h
out dx,al   


mov ax,00110110b		 ;Control Word for Timer 1 Counter 0 (Mode 3 : Square Wave Generator)
mov dx,2006h             ;16 bit binary count value written as LSB first followed by MSB
out dx,al            

mov ax,20h			 ;Loading count value into Timer 1 Counter 0
mov dx,2000h         ;Least Significant Byte = 20h
out dx,al            ;Most Significant Byte = 4Eh
mov ax,4Eh           ;4E20h = 20000d
mov dx,2000h         
out dx,al
...
mov ax,01110110b		;Control Word for Timer 1 Counter 1 (Mode 3 : Square Wave Generator)
mov dx,2006h            ;16 bit binary count value wriiten as LSB first followed by MSB
out dx,al

mov ax,	0E8h		;Loading count value into Timer 1 Counter 1
mov dx,2002h        ;Least Significant Byte = E8h    
out dx,al           ;Most Significant Byte = 03h
mov ax,03h          ;03E8h = 1000d
mov dx,2002h
out dx,al  


        
mov ax,10010010b		;Control Word for Timer 1 Counter 2 (Mode 1 : Triggerable One Shot Timer)
mov dx,2006h            ;8 bit Count Value is written into the LSB of the Counter 
out dx,al

;Count will be loaded to Timer 1 Counterr 2 Later
