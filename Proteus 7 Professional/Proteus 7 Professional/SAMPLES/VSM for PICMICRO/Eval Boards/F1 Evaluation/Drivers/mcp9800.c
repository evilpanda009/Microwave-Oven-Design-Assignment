...
static void write_one_byte(UINT8 reg, UINT8 data)
{
    ...
    d1 = reg;
    d1 &= 0x03; // guard the request register
    i2c_write_block(MCP9800_ADDRESS, 1, &d1, I2C_200K); // queue the write to setup the register
    i2c_write_block(MCP9800_ADDRESS, 1, &d2, I2C_200K); // queue the write to write the data
}
